...
export function POS({ products, cart, addToCart, removeFromCart, updateQuantity, onCheckout }: POSProps) {
  const [isSuccessModalOpen, setIsSuccessModalOpen] = useState(false);
  const [category, setCategory] = useState('All Items');

  const categories = ['All Items', 'Apparel', 'Footwear', 'Accessories', 'Electronics'];
  const filteredProducts = category === 'All Items' 
    ? products 
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {filteredProducts.map(product => (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1">
                  <h3 className="font-headline font-bold text-slate-900 tracking-tight leading-tight">{product.name}</h3>
                  <p className="text-xs text-slate-500 mb-4">SKU: {product.sku}</p>
                  <div className="mt-auto flex justify-between items-end">
                    <span className="text-lg font-headline font-extrabold text-primary">${product.price.toFixed(2)}</span>
      ...
      <section className="flex-1 flex flex-col bg-surface-container-low rounded-xl overflow-hidden shadow-2xl shadow-on-surface/5 border border-black/5">
        <div className="p-6 pb-4 border-b border-black/5">
          ...
          <p className="text-xs text-on-surface-variant/70 uppercase tracking-widest font-semibold">Transaction #082492</p>
        </div>
              ...
              <div key={item.id} className="flex gap-4 group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    ...
                    <button onClick={() => removeFromCart(item.id)} className="text-slate-400 hover:text-error transition-colors">
                      <X size={16} />
                    </button>
                  </div>
                  <div className="flex justify-between items-center mt-2">
                    <div className="flex items-center gap-3 bg-white px-2 py-1 rounded-md shadow-sm">
        ...
        <div className="bg-surface-container-high/50 p-6 space-y-3">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-on-surface-variant font-medium">Sales Tax (8%)</span>
        ...
        <div className="p-6 pt-0 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Button variant="secondary" className="flex items-center gap-2 py-3 border-2 border-primary text-primary bg-transparent">
      ...
      >
        <div className="text-center space-y-4">
          ...
          <div className="pt-6 flex gap-3">
            <Button variant="secondary" className="flex-1" onClick={() => setIsSuccessModalOpen(false)}>Print Receipt</Button>
            <Button className="flex-1" onClick={() => setIsSuccessModalOpen(false)}>Done</Button>
          </div>
        </div>
      </Modal>
    </div>
